...
void *AtenderCliente(void *socket) {
	...
	while (terminate == 0) {
		...
		char password[255];  // Almacena la contraseña del jugador
		int tercera[255];
		if (code != 0) {
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		if (sock_conn < 0) {
			...
			continue;  // No continuar si hay un error en la aceptación
		}
		
		pthread_t thread;  // Crear un hilo para atender al cliente
		pthread_create(&thread, NULL, AtenderCliente, (void *)&sock_conn);
		pthread_detach(thread);  // Desprender el hilo para que se gestione automáticamente
	}
